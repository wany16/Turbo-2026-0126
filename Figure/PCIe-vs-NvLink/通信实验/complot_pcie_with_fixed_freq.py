...
color_base = '#2CA02C'     # Green (Baseline 保持不变，或者也可以调浅 '#6BCB77')        
...
plt.plot(sm_counts, theoretical_latency, marker='^', markersize=MARKER_SIZE, linestyle='-.',
         color=color_theory, linewidth=LINE_WIDTH, label='Reserved SMs (Idle)')

# C. Comm Reserved (Parallelism Loss + Interference / Contention)
plt.plot(sm_counts, interference_latency, marker='s', markersize=MARKER_SIZE, linestyle='-',
...
plt.fill_between(sm_counts, theoretical_latency, interference_latency,
                 color='#FF6B6B', alpha=0.15, hatch='//', edgecolor='none')
# 添加文字说明
plt.text(25.7, 9.25, 
         "Resource Contention", ha='center', va='center',
         fontsize=font_xlabel['size']+5, color='black',
          alpha=1,
...
plt.fill_between(sm_counts, baseline_latency, theoretical_latency,
                 color='#4D96FF', alpha=0.1, hatch='\\', edgecolor='none')
